...
public class ArrList<E> {
	...
	public boolean add(E e) {
		if (arr.length == size) {
			resize(arr.length * 2);// 배열의 사이즈를 2배 증가시킨다. <==== (1. 여기 구현해보고 자주 사용되는 내용이면 함수로 extract , 리팩토링하기)
		}
...
//		} // capacity 확인 용도 테스트 코드

		if (size > 0 && size <= arr.length / 4) {
			resize(arr.length / 2);
			// 위 세 줄의 내용을 실행할 함수가 만들어져있다면 해당 함수 호출을 통해 배열의 사이즈를 1/2 로 줄인다. <==== (2. 여기
			// 구현해보기)
		}
		return removed;
	}
	...
	private void resize(int length) {
		E[] t = (E[]) (new Object[length]);// 배열의 크기를 반으로 줄이거나 2배 증가시킨다.
		for (int i = 0; i < size; i++) {
			t[i] = arr[i];
		} // 기존배열의 값을 새로 만든 배열로 옮긴다.
		arr = t; // 기존배열을 없애고 새로만든 배열을 arr 이 참조한다.
	}
